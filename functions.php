...
add_action('admin_post_nopriv_send_contact_email', 'handle_contact_form');
add_action('admin_post_send_contact_email', 'handle_contact_form');
...
function handle_contact_form()
{
    ...
    $headers = array(
        'From: PMB Website <no-reply@parkingmetalbox.com>',
        'Reply-To: ' . $name . ' <' . $email . '>'
    );
    ...
    // Redirect
    $redirect_url = !empty($_POST['redirect']) ? $_POST['redirect'] : home_url();
    if ($sent) {
...
function handle_bonus_reset()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_user'])) {
        ...
        wp_redirect(add_query_arg('bonus_paid', '1', wp_get_referer()));
        exit;
    }
}
...
function render_payout_settings_page()
{
    ...
    if (isset($_POST['payout_date_from']) && isset($_POST['payout_date_to'])) {
        update_option('payout_date_from', sanitize_text_field($_POST['payout_date_from']));
        update_option('payout_date_to', sanitize_text_field($_POST['payout_date_to']));
        echo '<div class="notice notice-success"><p>Zakres dat został zaktualizowany!</p></div>';
    }

    $payout_date_from = get_option('payout_date_from', '');
    $payout_date_to = get_option('payout_date_to', '');
?>
    <div class="wrap">
        ...
        <form method="POST">
            <table class="form-table">
                <tr>
                    <th scope="row">
                        <label for="payout_date_from">Data OD:</label>
                    </th>
                    <td>
                        <input
                            type="date"
                            ...
                            id="payout_date_from"
                            value="<?php echo esc_attr($payout_date_from); ?>"
                            required>
                    </td>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="payout_date_to">Data DO:</label>
                    </th>
                    <td>
                        <input
                            type="date"
                            name="payout_date_to"
                            id="payout_date_to"
                            value="<?php echo esc_attr($payout_date_to); ?>"
                            required>
                    </td>
                </tr>
            </table>
            <?php submit_button('Zapisz zakres'); ?>
        </form>

        <h3>Gdzie zmiana jest widoczna?</h3>
        <p>Zakres pojawi się w Panelu Ambasadora → sekcja „Dostępne bonusy”</p>
        <b>Termin następnej wypłaty: [data od] – [data do]</b>
    </div>
<?php
}
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="status-badge status-<?=
                                                                    strtolower(str_replace(' ', '-', $user->status_wyplaty ? $user->status_wyplaty : 'brak'))
                                                                    ?>">
                                    <?= $user->status_wyplaty ? esc_html($user->status_wyplaty) : 'brak' ?>
                                </span>
                            </td>
                            ...
                            <td>
                                <form method="post" class="inline-form">
                                    ...
                                    <input type="hidden" name="action" value="process_bonus_payment">
